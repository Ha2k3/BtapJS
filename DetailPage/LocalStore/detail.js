var product = [{
	...
	detail: 'Áo dài được may theo phong cách cổ kính, mang đậm vẻ đẹp truyền thống của dân tộc Việt Nam',
...
var listLocal = function () {
	...
	for (var i in product) {
		...
		listproduct += '</span>';
		listproduct += '<a>';
		listproduct += '<button class="btn btn-primary" data-toggle="modal" data-target="#proDetails" onclick="proDetail(' + i + ')">Detail</button>';
		listproduct += '</a>';
		listproduct += '</span>';
		listproduct += '</div>';
		listproduct += '</div>';
...
function proDetail(i) {
	...
	for (var key in product) {
		if (key == i) {
			...
			pDetail += '<div class="card-text">' + data.detail + '</div><br>';
			pDetail += '<span class="text-center add-to-cart btn">';
			pDetail += '<button class="btn btn-danger">' + '<i class="fa fa-cart-plus mt-3"> </i>' + '</button> </span>';
			pDetail += '</form>';
			break;
		}
	}
